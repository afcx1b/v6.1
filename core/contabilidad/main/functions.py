...
from core.contabilidad.main.models import Etiqueta, Cuenta, Movimiento
...
def valida_simple(movimiento_simple: dict, cuentas: QuerySet[Cuenta]) -> str:
    ...
    if pd.isnull(movimiento_simple['fecha']) or not isinstance(movimiento_simple['fecha'],
                                                               (pd.Timestamp, datetime.datetime, datetime.date)):
        return 'Fecha incorrecta'

    if pd.isnull(movimiento_simple['valor']) or not isinstance(movimiento_simple['valor'],
...
def valida_compleja(movimiento_complejo: dict, cuentas: QuerySet[Cuenta]) -> str:
    ...
    if pd.isnull(movimiento_complejo['fecha']) or not isinstance(movimiento_complejo['fecha'],
                                                                 (pd.Timestamp, datetime.datetime, datetime.date)):
        return 'Fecha incorrecta'

    if pd.isnull(movimiento_complejo['debe']) or not isinstance(movimiento_complejo['debe'],
                                                                (int, float, np.float64, np.int64)):
        return 'Debe es incorrecto'

    if pd.isnull(movimiento_complejo['haber']) or not isinstance(movimiento_complejo['haber'],
                                                                 (int, float, np.float64, np.int64)):
