...
var purchase = {
    ...
    calculateInvoice: function () {
        ...
        this.details.products.forEach(function (value, index, array) {
            ...
            value.subtotal = value.cant * parseFloat(value.cto);
...
$(function () {
    ...
    select_search_product.select2({
        ...
        templateResult: function (repo) {
            ...
            return $(
                ...
                '<b>Stock:</b> ' + stock + '<br>' +
                '<b>Costo:</b> <span class="badge badge-warning">$' + repo.cto + '</span>' +
                '</p>' +
                '</div>' +
                '</div>' +
                '</div>');
    ...
    $('#tblProducts tbody')
        ...
        .on('change', 'input[name="cant"]', function () {
            ...
            var tr = tblProducts.cell($(this).closest('td, li')).index();
            purchase.details.products[tr.row].cant = cant;
            purchase.calculateInvoice();
        ...
        submit_with_ajax(pathname, 'Notificación',
            '¿Estas seguro de realizar la siguiente acción?', parameters, function (response) {
                alert_action('Notificación', '¿Desea imprimir la boleta de venta?', function () {
                    window.open('/pos/purchase/invoice/pdf/' + response.id + '/', '_blank');
